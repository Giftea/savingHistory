...
import "./interfaces/SAVEHDelegateStorageV1.sol";
import "./interfaces/SAVEHEvents.sol";

contract SAVEHDelegate is
    SAVEHEvents,
    Initializable,
    OwnableUpgradeable,
    ReentrancyGuardUpgradeable,
    SAVEHDelegateStorageV1
{
    using SafeERC20Upgradeable for IERC20;

    /// @notice The name of this contract
    string public constant NAME = "SAVEH";
    ...
    function initialize(
        ...
        require(
            ...
            "SAVEH::initialize: timelock admin is not assigned to SAVEHDelegate"
        );
        require(
            _votingPeriod >= MIN_VOTING_PERIOD && _votingPeriod <= MAX_VOTING_PERIOD,
            "SAVEH::initialize: invalid voting period"
        );
        require(
            _votingDelay >= MIN_VOTING_DELAY && _votingDelay <= MAX_VOTING_DELAY,
            "SAVEH::initialize: invalid voting delay"
        );
        require(
            _proposalThreshold >= MIN_PROPOSAL_THRESHOLD &&
                _proposalThreshold <= MAX_PROPOSAL_THRESHOLD,
            "SAVEH::initialize: invalid proposal threshold"
        );
        require(_quorumVotes >= _proposalThreshold, "SAVEH::initialize: invalid quorum votes");
        timelock = TimelockInterface(_timelock);
        require(_token != address(0), "SAVEH::initialize: invalid _token address");
    ...
    function propose(
        ...
        require(
            ...
            "SAVEH::propose: proposer votes below proposal threshold"
        );
        require(
            _targets.length == _values.length &&
                _targets.length == _signatures.length &&
                _targets.length == _calldatas.length,
            "SAVEH::propose: proposal function information arity mismatch"
        );
        require(_targets.length != 0, "SAVEH::propose: must provide actions");
        require(_targets.length <= PROPOSAL_MAX_OPERATIONS, "SAVEH::propose: too many actions");
        ...
        if (_latestProposalId != 0) {
            ...
            require(
                ...
                "SAVEH::propose: one live proposal per proposer, found an already active proposal"
            );
            require(
                proposersLatestProposalState != ProposalState.Pending,
                "SAVEH::propose: one live proposal per proposer, found an already pending proposal"
            );
        }
    ...
    function queue(uint256 _proposalId) external {
        require(
            ...
            "SAVEH::queue: proposal can only be queued if it is succeeded"
        );
    ...
    function queueOrRevertInternal(
        ...
        require(
            ...
            "SAVEH::queueOrRevertInternal: identical proposal action already queued at eta"
        );
    ...
    function execute(uint256 _proposalId) external payable {
        require(
            ...
            "SAVEH::execute: proposal can only be executed if it is queued"
        );
        Proposal storage _proposal = proposals[_proposalId];
    ...
    function cancel(uint256 _proposalId) external {
        require(
            ...
            "SAVEH::cancel: cannot cancel executed proposal"
        );
        ...
        require(
            ...
            "SAVEH::cancel: proposer above threshold"
        );
    ...
    function state(uint256 _proposalId) public view returns (ProposalState) {
        require(proposalCount > _proposalId, "SAVEH::state: invalid proposal id");
        Proposal storage _proposal = proposals[_proposalId];
    ...
    function castVoteBySig(
        ...
        require(_v == 27 || _v == 28, "SAVEH::castVoteBySig: invalid v value");
        require(
            _s < 0x7FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF5D576E7357A4501DDFE92F46681B20A1,
            "SAVEH::castVoteBySig: invalid s value"
        );
        ...
        require(_signatory != address(0), "SAVEH::castVoteBySig: invalid signature");
        emit VoteCast(
            _signatory,
            _proposalId,
            _support,
    ...
    function castVoteInternal(
        ...
        require(
            ...
            "SAVEH::castVoteInternal: voting is closed"
        );
        require(_support <= 2, "SAVEH::castVoteInternal: invalid vote type");
        Proposal storage _proposal = proposals[_proposalId];
        Receipt storage _receipt = proposalReceipts[_proposalId][_voter];
        require(!_receipt.hasVoted, "SAVEH::castVoteInternal: voter already voted");
    ...
    function _setVotingDelay(uint256 _newVotingDelay) external virtual onlyOwner {
        require(
            ...
            "SAVEH::_setVotingDelay: invalid voting delay"
        );
    ...
    function _setQuorumVotes(uint256 _newQuorumVotes) external onlyOwner {
        require(
            ...
            "SAVEH::_setQuorumVotes: invalid quorum votes"
        );
    ...
    function _setVotingPeriod(uint256 _newVotingPeriod) external virtual onlyOwner {
        require(
            ...
            "SAVEH::_setVotingPeriod: invalid voting period"
        );
    ...
    function _setProposalThreshold(uint256 _newProposalThreshold) external onlyOwner {
        require(
            ...
            "SAVEH::_setProposalThreshold: invalid proposal threshold"
        );
    ...
    function _setReleaseToken(IHasVotes _newReleaseToken) external onlyOwner {
        require(
            ...
            "SAVEH::_setReleaseToken: releaseToken and token must be different"
        );
