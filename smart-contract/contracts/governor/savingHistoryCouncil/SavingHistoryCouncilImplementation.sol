...
import "./interfaces/SavingHistoryCouncilStorageV1.sol";

contract SavingHistoryCouncilImplementation is
    Initializable,
    OwnableUpgradeable,
    ReentrancyGuardUpgradeable,
    SavingHistoryCouncilStorageV1
{
    ...
    modifier onlyMember() {
        require(members[msg.sender] == true, "SAVEH:: Not a member");
        _;
    }
    ...
    function initialize(
        ...
        require(_quorumVotes >= 1, "SAVEH::initialize: invalid proposal threshold");
        require(_quorumVotes <= _members.length, "SAVEH::initialize: params mismatch");
    ...
    /**
     * @notice Function used to add new members to the savingHistory Council.
     * @param _member Member address.
     */
    function addMember(address _member) external onlyOwner {
        require(members[_member] == false, "SAVEH::addMember: already a member");
    ...
    /**
     * @notice Function used to remove members from the savingHistory Council.
     * @param _member Member address.
     */
    function removeMember(address _member) external onlyOwner {
        require(members[_member] == true, "SAVEH::removeMember: not a member");
    ...
    function propose(
        ...
        require(_signatures.length != 0, "SAVEH::propose: must provide actions");
        require(_signatures.length <= PROPOSAL_MAX_OPERATIONS, "SAVEH::propose: too many actions");
    ...
    function execute(uint256 _proposalId) external onlyMember payable {
        ...
        for (; _i < _numberOfActions; _i++) {
            ...
            require(_success, "SAVEH::execute: Transaction execution reverted.");
        }
    ...
    function cancel(uint256 _proposalId) external onlyMember {
        ...
        Proposal storage _proposal = proposals[_proposalId];
        require(msg.sender == _proposal.proposer, "SAVEH::cancel: proposer not allowed");
    ...
    function castVoteInternal(
        ...
        require(_receipt.hasVoted == false, "SAVEH::castVoteInternal: voter already voted");
        uint96 _votes = 1;
    ...
    function setQuorumVotes(uint256 _newQuorumVotes) external onlyOwner {
        require(_newQuorumVotes >= 1, "SAVEH::_setQuorumVotes: invalid quorum votes");
