    ...
    DonationMinerStorageV1
{
     ...
     * @param _SAVEH                 Address of the SAVEH Token
     * @param _treasury             Address of the Treasury
     * @param _firstRewardPerBlock  Number of SAVEHs given for each block
    ...
    function initialize(
        ...
        require(address(_usdc) != address(0), "DonationMiner::initialize: usdc address not set");
        require(address(_SAVEH) != address(0), "DonationMiner::initialize: SAVEH address not set");
    ...
    /**
     * @notice Returns the amount of usdc donated by a user in a reward period
     *
     * @param _period number of the reward period
     * @param _donor address of the donor
     * @return uint256 amount of usdc donated by the user in this reward period
     */
    ...
    /**
     * @notice Returns the amount of SAVEH staked by a user at the and of the reward period
     *
     * @param _period reward period number
     * @param _donor address of the donor
     * @return uint256 amount of SAVEH staked by a user at the and of the reward period
     */
     ...
     *
     * @param _newStakingDonationRatio    Number of tokens that need to be staked to be counted as 1 SAVEH donated
     */
    ...
    /**
     * @notice Transfers usdc tokens to the treasury contract
     *
    ...
    function donate(
        ...
        require(
            _token == usdc || treasury.isToken(address(_token)),
    ...
    function donateToCommunity(
        ...
        require(
            address(_token) == address(_community.usdc()),
    ...
    function claimRewards() external override whenNotPaused whenStarted nonReentrant {
        ...
        IERC20Upgradeable(address(SAVEH)).safeTransfer(msg.sender, _claimAmount);
    ...
    function stakeRewards() external override whenNotPaused whenStarted nonReentrant {
        ...
        SAVEH.approve(address(staking), _stakeAmount);
        ...
        returns (uint256)
    {
        ...
        if (_claimAmount > SAVEH.balanceOf(address(this))) {
            _claimAmount = SAVEH.balanceOf(address(this));
        }
     ...
     * @return _lastDonorStakeAmount uint256 number of SAVEHs that are staked by the donor at the end of _lastPeriodNumber
     */
        ...
        returns (uint256)
    {
        ...
        uint256 totalStakeAmount = staking.SSAVEH().totalSupply();
